...
const addDatabaseIndexes = async () => {
    try {
        ...
        }, { name: 'user_bookings_compound_idx' });
        
        // 🚀 CRITICAL: Individual field indexes for fast filtering
        await User.collection.createIndex({ 'bookings.coachId': 1 }, { name: 'user_bookings_coachId_idx' });
        await User.collection.createIndex({ 'bookings.paymentStatus': 1 }, { name: 'user_bookings_paymentStatus_idx' });
        
        // 🔥 NEW: Index for lastActive field for performance optimization
        await User.collection.createIndex({ lastActive: -1 }, { name: 'user_lastActive_idx' });
        
        // Index for coach lookups and specialty filtering
        await Coach.collection.createIndex({ '_id': 1, 'specialties': 1 }, { name: 'coach_id_specialties_idx' });
        await Coach.collection.createIndex({ 'specialties': 1 }, { name: 'coach_specialties_idx' });
        
        // Username lookups (login/profile) - with unique names to avoid conflicts
        await User.collection.createIndex({ 'username': 1 }, { name: 'user_username_idx' });
        await Coach.collection.createIndex({ 'username': 1 }, { name: 'coach_username_idx' });
...
app.post('/api/membership-application/:id/approve', async (req, res) => {
  try {
    ...
    // 🚀 AUTO-DELETE: Clear image after approval to save storage
    const application = await MembershipApplication.findByIdAndUpdate(
      ...
      { 
        ...
        startDate, 
        expirationDate,
        proof: 'Image deleted after verification for storage optimization' // Clear large image
      },
      { new: true }
    );
    
    console.log(`🗑️ Membership application image deleted for ${req.params.id} (approved)`);
...
app.post('/api/membership-application/:id/reject', async (req, res) => {
  try {
    // 🚀 AUTO-DELETE: Clear image after rejection to save storage
    const application = await MembershipApplication.findByIdAndUpdate(
      req.params.id,
      { 
        status: 'rejected', 
    ...
    console.log(`🗑️ Membership application image deleted for ${req.params.id} (rejected)`);
    if (!application) return res.status(404).json({ success: false, message: 'Application not found' });
